...
export default function AddSubjectModal({ isOpen, onClose, onSuccess }: AddSubjectModalProps) {
    ...
    const fetchData = async () => {
        setLoadingData(true);
        try {
            // Fetch both classes and teachers with request() wrapping to handle data envelopes
            const [classesData, teachersData] = await Promise.all([
                request<any[]>(() => api.get('/school/classes')),
                request<any[]>(() => api.get('/school/teachers'))
            ]);

            // Safely extract from typical axios wrapped responses
            // `request` helper usually returns the unwrapped data, but double check envelope just in case
            const extractData = (res: any) => Array.isArray(res) ? res : (res.classes || res.data || []);
            
            const cData = extractData(classesData);
            setClasses(cData);
            
            // Map teachers to uniform {id, name} structure
            const tData = extractData(teachersData);
