...
import api, { request, formatApiError } from '@/lib/api';
...
const AddScheduleForm = ({ examId, onSuccess }: { examId: string, onSuccess: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // POST /school/exam-schedules — examId in body, not in path
      await request(() => api.post('/school/exam-schedules', {
        examId,
        classId,
        subjectId,
        date,
      ...
      // Show the real backend message (e.g. conflict details) instead of a generic alert
      setFormError(formatApiError('Failed to add schedule', err));
...
export default function ExamDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      try {
        setError(null);
        // GET /school/exam-schedules?examId=:examId  (matches the backend route that exists)
        const data = await request<Schedule[]>(() => api.get('/school/exam-schedules', { params: { examId } }));
